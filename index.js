...
app.post("/api/register/kolkata", async (req, res) => {
...
app.post("/api/register/lucknow", async (req, res) => {
  // Check if a user with the same ID already exists
  const existingUser = await Student.findOne({ ID: req.body.id });
  if (existingUser) {
    return res.status(400).json({ error: "User ID already exists" });
  }
...
app.post("/api/register/ahmedabad", async (req, res) => {
  // Check if a user with the same ID already exists
  const existingUser = await Examee.findOne({ ID: req.body.id });
  if (existingUser) {
    return res.status(400).json({ error: "User ID already exists" });
  }
...
app.get("/api/users/kolkata", async (req, res) => {
  try {
...
app.get("/api/users/lucknow", async (req, res) => {
  try {
...
app.get("/api/users/ahmedabad", async (req, res) => {
  try {
