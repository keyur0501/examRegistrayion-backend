...
app.post("/api/register/Chandigarh", async (req, res) => {
  // Check if a user with the same ID already exists
  const existingUser = await Person.findOne({ ID: req.body.id });
  if (existingUser) {
    return res.status(400).json({ error: "User ID already exists" });
  }
...
app.post("/api/register/Delhi", async (req, res) => {
  // Check if a user with the same ID already exists
  const existingUser = await Customer.findOne({ ID: req.body.id });
  if (existingUser) {
    return res.status(400).json({ error: "User ID already exists" });
  }
...
app.post("/api/register/Guwahati", async (req, res) => {
  // Check if a user with the same ID already exists
  const existingUser = await Buyer.findOne({ ID: req.body.id });
  if (existingUser) {
    return res.status(400).json({ error: "User ID already exists" });
  }
...
app.post("/api/register/Chennai", async (req, res) => {
  // Check if a user with the same ID already exists
  const existingUser = await Participant.findOne({ ID: req.body.id });
  if (existingUser) {
    return res.status(400).json({ error: "User ID already exists" });
  }
